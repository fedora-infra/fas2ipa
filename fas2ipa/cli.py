...
@click.option("--only-members", is_flag=True, help="Only map users/sponsors to groups and ignore updating user entities")
def cli(skip_groups, only_members):
    ...
    for section, keys in INPUT_IF_EMTPY.items():
        for key in keys:
            ...
            config[section][key] = click.prompt(f"Enter {key} for {section}", hide_input=is_password)
    ...
    if not skip_groups:
        ...
        fas_groups = [g for g in fas_groups['groups'] if g['name'] not in config["ignore_groups"]]
        ...
        for group in progressbar.progressbar(fas_groups, redirect_stdout=True):
            ...
            try:
                ...
                if e.message == 'group with name "%s" already exists' % group["name"]:
                    ipa.group_mod(group['name'], description=group['display_name'].strip(),
                        fasgroup=True, fasurl=url, fasmailinglist=mailing_list, fasircchannel=irc_string)
                    print('UPDATED')
                    groups_edited += 1
                else:
    ...
    def stats():
        ...
        print('')
        print(f'Total FAS groups: {groups_counter}. Total groups added in FreeIPA: { groups_added + groups_edited }')
